...
export class TerminosCondicionesService {
    ...
    TerminosCondicionesLista(): Observable<CORP_TerminosCondiciones[]> {
        return this.authHttp.get(this.grupoUrl + '/TerminosCondicionesLista')
            .map((res) => this.extractData(res))
            .catch(this.authHttp.handleError);
    }

    TerminosCondicionesPorID(TerminosCondicionesID: number): Observable<CORP_TerminosCondiciones[]> {
        return this.authHttp.get(this.grupoUrl + '/TerminosCondicionesPorID?TerminosCondicionesPorID=' + TerminosCondicionesID)
            .map((res) => this.extractData(res))
            .catch(this.authHttp.handleError);
    }
    ...
    TerminosCondicionesCrearActualizar(terminoscondiciones: CORP_TerminosCondiciones): Observable<boolean> {
        ...
        return this.authHttp.post(this.grupoUrl + '/TerminosCondicionesCrearActualizar', body)
            .map(this.extractData)
            .catch(this.authHttp.handleError);
    }

    TerminosCondicionesPublicar(TerminosCondicionesID: number): Observable<boolean> {
        return this.authHttp.get(this.grupoUrl + '/TerminosCondicionesPublicar?TerminosCondicionesID=' + TerminosCondicionesID)
            .map((res) => this.extractData(res))
            .catch(this.authHttp.handleError);
    }
