...
@Component({
    selector: 'agregarEditarVendendor',
    providers: [ServicioVentasService, PersonaService, ConstantesComercialService, EmpresaService, CatalogoComercialService],
    templateUrl: 'agregarEditarVendedor.component.html'
})

export class AgregarEditarVendedorComponent implements OnInit, OnDestroy {
    ...
    constructor(public domSanitizer: DomSanitizer, private authService: AuthService, private elementRef: ElementRef,
        public directorService: VendedoresComponent, public ventasService: ServicioVentasService, public personaService: PersonaService,
        ...
        public subtipoService: SubtipoService, public regionService: RegionService, public genericosService: GenericosService, public activatedRoute: ActivatedRoute) {
        ...
        this.suscription = this.directorService.selectDirector$.subscribe(
            (vendedor) => {
                this.setear();  
                if (vendedor != undefined && vendedor.TipoTransaccion != undefined) {
                                   
                    this.director.TipoTransaccion = vendedor.TipoTransaccion; 

                    if (this.director.TipoTransaccion == this.constCom.TIPO_TRANSACCION_INGRESAR) 
                    {
                        this.loadDataFromParent(this.activatedRoute.snapshot.params['director']); 
                        this.cargarCombos();                   
                    }
                    if (this.director.TipoTransaccion == this.constCom.TIPO_TRANSACCION_ACTUALIZAR)
                    {
                        this.filtroAgenteVenta.CodigoAgenteVenta = vendedor.Codigo;
                        ...
                        this.ventasService.GetAgentesVentaByFiltersPaginated(this.filtroAgenteVenta, 10).subscribe(
                            res => {                           
                                ...
                                this.director = this.setearAManoVendedor(res[0]);                                       
                                this.director.TipoTransaccion = vendedor.TipoTransaccion;     
                                ...
                                if(this.director.ProgramaCreacion == 'Aplicacion_WEB' || this.director.ProgramaModificacion == 'Aplicacion_WEB'){                                  
                                    ...
                                    this.loadDataFromParent(vendedor.CodigoDirector); 
                                }                                   
                                else{
                                    this.loadDataFromParent(vendedor.CodigoDirector); 
                                    this.cargarCombos();
    ...
    setearAManoVendedor(vendedor: DirectorVendedorEntity){
        let vendedorNuevo: DirectorVendedorEntity = new DirectorVendedorEntity();
        vendedorNuevo.AplicaCorporativo = vendedor.AplicaCorporativo; 
        vendedorNuevo.Codigo = vendedor.Codigo; 
        vendedorNuevo.CodigoDirector = vendedor.CodigoDirector; 
        vendedorNuevo.CodigoSubtipo = vendedor.CodigoSubtipo; 
        vendedorNuevo.CodigoVendedor = vendedor.CodigoVendedor; 
        vendedorNuevo.CumplimientoQueja = vendedor.CumplimientoQueja; 
        vendedorNuevo.FechaIngreso = vendedor.FechaIngreso; 
        vendedorNuevo.FechaProduccionAgente = vendedor.FechaProduccionAgente; 
        vendedorNuevo.FechaSalida = vendedor.FechaSalida; 
        vendedorNuevo.Estado = vendedor.Estado;
        vendedorNuevo.Nombre = vendedor.Nombre; 
        vendedorNuevo.LoginUsuario = vendedor.LoginUsuario;     
        vendedorNuevo.PorcentajePersistencia = vendedor.PorcentajePersistencia; 
        vendedorNuevo.ProgramaCreacion = vendedor.ProgramaCreacion; 
        vendedorNuevo.ProgramaModificacion = vendedor.ProgramaModificacion; 
        vendedorNuevo.TipoAgenteVenta = vendedor.TipoAgenteVenta; 
        vendedorNuevo.UsuarioDirectorioActivo = vendedor.UsuarioDirectorioActivo; 

        return vendedorNuevo; 
    }

    loadDataFromParent(codigoDirector: number){
        let filtroDirector : FiltroAgenteVenta = new FiltroAgenteVenta(); 
        filtroDirector.CodigoAgenteVenta = codigoDirector; 
        filtroDirector.Tipo = 'Director';

        this.ventasService.GetAgentesVentaByFiltersPaginated(filtroDirector, 10).subscribe(
            res => {                           
                this.directorFromParent = res[0];     

                if (this.listaRegiones.length == 0)
                    this.loadRegiones();

                this.director.Region = this.directorFromParent.Region; 

                if (this.listaSucursales.length == 0)
                    this.cargarSucursal();

                this.director.CodigoSucursal = this.directorFromParent.CodigoSucursal; 

                if (this.listaSalas.length == 0)
                    this.loadSalas(); 

                this.director.CodigoSala = this.directorFromParent.CodigoSala; 
            }
        );                                  
    }
    ...
    loadTipo() {
        this.tipoService.getTodosTiposHijos().subscribe((result) => {
    ...
    confirmarCrearDirector() {
        swal({
            title: "",
            text: "<h3>Está seguro que desea ingresar a " + this.director.Nombre + " como Vendedor</h3>",
            html: true,
            type: "info",
    ...
    crearDirector() {
        this.director.CodigoDirector = this.directorFromParent.Codigo;
        this.director.TipoAgenteVenta = 'Vendedor'; 
        this.ventasService.CrearAgenteVenta(this.director).subscribe(
            result => {
                this.authService.showSuccessPopup("El Vendedor " + this.director.Nombre + " han sido ingresado con el código " + result);
                this.director = new DirectorVendedorEntity();
                this.persona = new PersonaEntity();
                this.volverMenuPrincipal();
            },
