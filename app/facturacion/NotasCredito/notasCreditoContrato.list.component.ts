...
@Component({
    ...
    templateUrl: 'notasCreditoContrato.list.template.html'
})

export class NotasCreditoContratoListComponent implements OnInit {
    ...
    constructor(private elementRef: ElementRef, private router: Router, private chRef: ChangeDetectorRef,
        ...
        public contratoService: ContratoService, private constantService: ConstantService) {
    }
    ...
    filtrar(): void {
        this.contratoService.getListaContratosPaginated(this.filter)
            .subscribe(contratos => {
    ...
    inicializarPanelSobres(selected: ContratoEntityList): void {
        ...
        jQuery("#divPanelNotasCredito").collapse("show");
        ...
        // scroll top
        this.chRef.detectChanges();
        var inipos = jQuery("#divPanelNotasCredito").position().top;
