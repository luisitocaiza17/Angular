...
@Component({
    selector: 'notasCreditoLote',
    providers: [TareasProgramadasFacturacionService, GenericosService],
    templateUrl: 'notasCreditoLote.form.template.html'
})

export class NotasCreditoLoteForm {
    ...
    emitir() {
        this.tareasProgramadasFacturacionService.emitirNotasCreditoLote(this.cronTab)
            .subscribe(result => {
    ...
    eliminarTarea() {
        this.genericosService.eliminarTareaProgramada("EmitirNotasCreditoLote")
            .subscribe(result => {
                if(result)
